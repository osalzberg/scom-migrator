        ...
        template = {
            ...
            "parameters": {
                ...
                "customLogPath": {
                    ...
                    "metadata": {"description": "Path pattern for custom log files generated by migrated scripts (use forward slashes)"}
                }
            },
            "variables": {
            ...
            "resources": [
                {
                    "condition": "[and(not(empty(parameters('workspaceResourceId'))), not(empty(parameters('dataCollectionEndpointId'))))]",
                    "type": "Microsoft.Insights/dataCollectionRules",
                            ...
                            "logFiles": [
                                {
                                    ...
                                    "streams": ["[variables('customStreamName')]"],
    ...
    def generate_complete_deployment(
        ...
        for res in custom_log_dcr.get("resources", []):
            ...
            if "properties" in custom_res:
                ...
                # Use the variable for DCE
                props["dataCollectionEndpointId"] = "[parameters('dataCollectionEndpointId')]"
                # Fix stream declarations to use ARM variable
                props["streamDeclarations"] = {
                    "[variables('customStreamName')]": {
                        "columns": [
                            {"name": "TimeGenerated", "type": "datetime"},
                            {"name": "RawData", "type": "string"},
